...
#Region "Initilize and Settings"
    ...
    Private Sub Initialize()
        ...
        UserSettings.AcceptedImageFormat = New List(Of Imaging.PixelFormat)
        UserSettings.ExcludedImageFormat = New List(Of Imaging.PixelFormat)

        UserSettings.AcceptedImageFormat.Add(PixelFormat.Format16bppRgb555)
        UserSettings.AcceptedImageFormat.Add(PixelFormat.Format16bppRgb565)
        UserSettings.AcceptedImageFormat.Add(PixelFormat.Format24bppRgb)
        UserSettings.AcceptedImageFormat.Add(PixelFormat.Format32bppArgb)
        UserSettings.AcceptedImageFormat.Add(PixelFormat.Format32bppPArgb)
        UserSettings.AcceptedImageFormat.Add(PixelFormat.Format32bppRgb)
        UserSettings.AcceptedImageFormat.Add(PixelFormat.Format48bppRgb)
        ...
        Try
            If Directory.Exists(UserSettings.AmhOcrTempFolder) Then
                Directory.Delete(UserSettings.AmhOcrTempFolder, True)
            End If
        Catch ex As Exception

        End Try

        If Directory.Exists(UserSettings.AmhOcrConvFolder) = False Then
            Directory.CreateDirectory(UserSettings.AmhOcrConvFolder)
        End If

        If Directory.Exists(UserSettings.ProjectMainFolder) = False Then
            Directory.CreateDirectory(UserSettings.ProjectMainFolder)
        End If


        If Directory.Exists(UserSettings.AmhOcrTempFolder) = False Then
            Directory.CreateDirectory(UserSettings.AmhOcrTempFolder)
        End If

        If Directory.Exists(UserSettings.AmhOcrDataFolder) = False Then
            Directory.CreateDirectory(UserSettings.AmhOcrDataFolder)
        End If

        Dim otherfontfile = Path.Combine(Environment.CurrentDirectory, "Data.Fonts\times.ttf")
        Dim amhfontfile = Path.Combine(Environment.CurrentDirectory, "Data.Fonts\PGUNICODE1.TTF")
        ...
        If Not Instfont.Families.Any(Function(X) X.Name = "Power Geez Unicode1") Then
            ...
            UserSettings.AmhocrFont = New Font(fmly, 11, FontStyle.Regular)

        Else

            UserSettings.AmhocrFont = New Font("Power Geez Unicode1", 11, FontStyle.Regular)
        ...
        If Not Instfont.Families.Any(Function(X) X.Name = "Times New Roman") Then
            ...
            UserSettings.DefaultocrFont = New Font(fmly, 11, FontStyle.Regular)

        Else

            UserSettings.DefaultocrFont = New Font("Times New Roman", 11, FontStyle.Regular)

        End If

        UserSettings.tesspath = IO.Path.Combine(Environment.CurrentDirectory, "Tesseract\tessdata\")
        ...
        ' UserSettings.EditMode, only two mode currently implimented
        ...
        UserSettings.Language = UserSettings.PrefLanguage
        UserSettings.TimeOut = UserSettings.PrefTimeOut
        UserSettings.MaxBatch = UserSettings.PrefMaxBatch
        UserSettings.SpellErrorColor = UserSettings.PrefSpellErrorColor
        UserSettings.UserSpelledColor = UserSettings.PrefUserSpelledColor
        UserSettings.Binaries = UserSettings.PrefBinary

        If AvailabelLangs.Contains(UserSettings.Language) Then
            isBusy = True

            Dim langindx = AvailabelLangs.IndexOf(UserSettings.Language)
            AvailabelLangs.RemoveAt(langindx)
            AvailabelLangs.Insert(0, UserSettings.Language)

            CmbLang.Items.RemoveAt(langindx)
            CmbLang.Items.Insert(0, UserSettings.Language)
            CmbLang.SelectedIndex = 0
            isBusy = False

        Else

            UserSettings.Language = AvailabelLangs(0)
            UserSettings.PrefLanguage = UserSettings.Language

        End If


        If UserSettings.Language = "amh" Then
            UserSettings.ocrFont = UserSettings.AmhocrFont.Clone
        Else
            UserSettings.ocrFont = UserSettings.DefaultocrFont.Clone
        End If
        ...
        AddHandler ImagelistContextMenu.Opening,
            ...
            Sub(s, e)
                ...
                Else
                    ...
                    Dim imgFile = FilePaths(ListOpenedImages.SelectedIndices(0))

                    If EditorPicBox.FileName = imgFile Then

                        TreeContextOpen.Enabled = False


                        If HocrPages.Any(Function(X) X.ImageName = imgFile AndAlso X.Recognized = True) Then
                            TreeContextReset.Enabled = True
                            TreeContextSaveAs.Enabled = True
                            TreeContextOpenDetect.Enabled = False
                        Else
                            TreeContextReset.Enabled = True
                            TreeContextSaveAs.Enabled = False
                            TreeContextOpenDetect.Enabled = True
                        End If
                    Else
                        TreeContextOpen.Enabled = True
                        TreeContextOpenDetect.Enabled = True
    ...
    Private Sub Creat_SavePreference()
        ...
        Dim PrefFilePath = Path.Combine(UserSettings.AmhOcrDataFolder, "preference.amhocrsetting")

        Dim txtLines(8) As String
        txtLines.Initialize()
        txtLines(0) = UserSettings.PrefLanguage
        txtLines(1) = UserSettings.PrefTimeOut.ToString
        txtLines(2) = UserSettings.PrefMaxBatch.ToString
        txtLines(3) = UserSettings.PrefSpellErrorColor.ToArgb.ToString
        txtLines(4) = UserSettings.PrefUserSpelledColor.ToArgb.ToString
        txtLines(5) = CInt(UserSettings.PrefBinary).ToString
        txtLines(6) = CInt(UserSettings.RemoveWhiteListChar).ToString
        txtLines(7) = CInt(UserSettings.NormalizeChar).ToString
        txtLines(8) = CInt(UserSettings.NormalizeNumerics).ToString
        Try
            ...
            Try
                ...
                For ln As Integer = 0 To txtLines.Count - 1 Step 1
                    ...
                    If ln = 0 Then
                        ...
                        Dim OldPref = UserSettings.PrefLanguage

                        UserSettings.PrefLanguage = txtLines(ln)

                        If Not AvailabelLangs.Contains(UserSettings.PrefLanguage) Then
                            UserSettings.PrefLanguage = OldPref
                        End If


                    ElseIf ln = 1 Then

                        UserSettings.PrefTimeOut = CInt(txtLines(ln))

                    ElseIf ln = 2 Then

                        UserSettings.PrefMaxBatch = CInt(txtLines(ln))

                    ElseIf ln = 3 Then

                        UserSettings.PrefSpellErrorColor = Color.FromArgb(CInt(txtLines(ln)))


                    ElseIf ln = 4 Then

                        UserSettings.PrefUserSpelledColor = Color.FromArgb(CInt(txtLines(ln)))

                    ElseIf ln = 5 Then

                        UserSettings.PrefBinary = CInt(txtLines(ln))

                    ElseIf ln = 6 Then

                        UserSettings.RemoveWhiteListChar = CInt(txtLines(ln))

                    ElseIf ln = 7 Then

                        UserSettings.NormalizeChar = CInt(txtLines(ln))

                    ElseIf ln = 8 Then

                        UserSettings.NormalizeNumerics = CInt(txtLines(ln))
    ...
    Private Sub ApplyUserPreferances()
        ...
        AvailabelLangs.Remove(UserSettings.PrefLanguage)
        AvailabelLangs.Insert(0, UserSettings.PrefLanguage)
        ...
        If HocrPages IsNot Nothing Then
            ...
            For pg As Integer = 0 To HocrPages.Count - 1 Step 1
                ...
                If HocrP.Recognized = False OrElse HocrP.AllocrCarea.Count = 0 Then
                    ...
                    If pgSetting.Binaries <> UserSettings.Binaries Then
    ...
    Public Sub RefreshLanguage()
        ...
        Dim Files = Directory.GetFiles(UserSettings.tesspath).OrderBy(Function(X) X)
    ...
    Public Sub EditModeChanged(ByVal sender As Object, ByVal e As EventArgs)
        If isBusy = False Then
            ...
            If UserSettings.EditMode = ocrEditMode.WordEdit Then
                cmbEditMode.SelectedIndex = 0
            ElseIf UserSettings.EditMode = ocrEditMode.LineEdit Then
    ...
    Private Sub CmbLang_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles CmbLang.SelectedIndexChanged
        If (AvailabelLangs.Count > 0) AndAlso (isBusy = False) Then
            ...
            If AvailabelLangs.Contains(CmbLang.SelectedItem) Then
                ...
                UserSettings.Language = _Language
    ...
    Private Sub PreferencesToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PreferencesToolStripMenuItem.Click
        ...
        If isBusy = False AndAlso HocrPicBox.isBusy = False AndAlso AvailabelLangs.Count > 0 Then
            ...
            userPref.cmbLang.SelectedIndex = AvailabelLangs.IndexOf(UserSettings.PrefLanguage)
            userPref.numTimeout.Value = CDec(UserSettings.PrefTimeOut / (60 * 1000))
            userPref.numThreadNumber.Value = UserSettings.PrefMaxBatch
            userPref.lblSpellColor.BackColor = UserSettings.PrefSpellErrorColor
            userPref.lblUserColor.BackColor = UserSettings.PrefUserSpelledColor
            userPref.chkBinary.Checked = UserSettings.PrefBinary
            userPref.chkWhitelisted.Checked = UserSettings.RemoveWhiteListChar
            userPref.chkNormChar.Checked = UserSettings.NormalizeChar
            userPref.chkNormNumerics.Checked = UserSettings.NormalizeNumerics

            If userPref.ShowDialog(Me) = DialogResult.OK Then

                UserSettings.PrefLanguage = userPref.cmbLang.Items(userPref.cmbLang.SelectedIndex)
                UserSettings.PrefTimeOut = userPref.numTimeout.Value * 60 * 1000
                UserSettings.PrefMaxBatch = CInt(userPref.numThreadNumber.Value)
                UserSettings.PrefSpellErrorColor = userPref.lblSpellColor.BackColor
                UserSettings.PrefUserSpelledColor = userPref.lblUserColor.BackColor
                UserSettings.PrefBinary = userPref.chkBinary.Checked
                UserSettings.RemoveWhiteListChar = userPref.chkWhitelisted.Checked
                UserSettings.NormalizeChar = userPref.chkNormChar.Checked
                UserSettings.NormalizeNumerics = userPref.chkNormNumerics.Checked
...
#Region "Main User interfaces"
    ...
    Private Sub cmbEditMode_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbEditMode.SelectedIndexChanged
        If isBusy = False Then
            If cmbEditMode.SelectedIndex = 0 Then
                UserSettings.EditMode = ocrEditMode.WordEdit
            ElseIf cmbEditMode.SelectedIndex = 1 Then
                UserSettings.EditMode = ocrEditMode.LineEdit
            ElseIf cmbEditMode.SelectedIndex = 2 Then
                UserSettings.EditMode = ocrEditMode.ParagraphEdit
            End If
            ...
            If HocrPicBox.Image IsNot Nothing Then
                ...
                If HocrPicBox.HocrPage IsNot Nothing AndAlso
                   HocrPicBox.State = controlState.None Then
                    ...
                    HocrPicBox.drawingObject = New DrawingObject
    ...
    Private Sub btnboxdraw_Click(sender As Object, e As EventArgs) Handles btnboxdraw.Click
        ...
        If isBusy = False AndAlso HocrPicBox.isBusy = False Then
            ...
            If HocrPicBox.HocrPage IsNot Nothing Then
                ...
                HocrPicBox.drawingObject = New DrawingObject
                HocrPicBox.drawingObject.type = DrawingType.Rectangle
                HocrPicBox.State = controlState.DrawStart
                HocrPicBox.Cursor = Cursors.Cross
            ...
            ElseIf EditorPicBox.Image IsNot Nothing Then
                ...
                EditorPicBox.drawingObject.type = DrawingType.Rectangle

                EditorPicBox.State = controlState.DrawStart
    ...
    Private Sub CropImageArea(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If sender.GetType.Name = "ImageEditControl" Then
            ...
            CropImage(EditorPicBox.drawingObject.BoundingBox)

        ElseIf sender.GetType.Name = "HocrEditControl" Then

            CropImage(HocrPicBox.drawingObject.BoundingBox)
    ...
    Private Sub btnCrop_Click(sender As Object, e As EventArgs) Handles btnCrop.Click
        ...
        If EditorPicBox.State = controlState.ObjectSelection AndAlso
                        Not EditorPicBox.drawingObject.BoundingBox.IsEmpty Then
    ...
    Private Sub btnCropBoundary_Click(sender As Object, e As EventArgs) Handles btnCropBoundary.Click


        If isRecognizOpen = False AndAlso isBusy = False AndAlso
            EditorPicBox.Image IsNot Nothing AndAlso
            EditorPicBox.State = controlState.None Then

            SetGetPageBoundary()


        ElseIf isRecognizOpen = False AndAlso isBusy = False AndAlso
            EditorPicBox.Image IsNot Nothing AndAlso
            EditorPicBox.State = controlState.ObjectSelection AndAlso
            EditorPicBox.drawingObject.type = DrawingType.Quadrilateral Then

            If EditorPicBox.drawingObject.Points.Count = 4 Then

                EditorPicBox.ZoomReset()
                isBusy = True
                Crop_and_GetPageBoundary()
    ...
    Public Sub OCRObjectSelection(ByVal sender As Object, ByVal e As BoundingBoxArg)
        ...
        Else
            ...
            If EditorPicBox.drawingObject.BoundingBox = Rectangle.Empty Then
...
#Region "UI View Control"
    ...
    Private Sub btnBackground_Click(sender As Object, e As EventArgs) Handles btnBackground.Click
        ...
        ElseIf UserSettings.OCRbackgroundView = BackgroundMode.OriginalImage Then
            ...
            UserAreaEditedImageToolStripMenuItem.Checked = False
            UserAreaOriginalAreaToolStripMenuItem.Checked = False
        ElseIf UserSettings.OCRbackgroundView = BackgroundMode.UserEditedImageArea Then
    ...
    Private Sub MenuItemOriginalmageView_Click(sender As Object, e As EventArgs) Handles MenuItemOriginalmageView.Click
        ...
        UserSettings.OCRbackgroundView = BackgroundMode.OriginalImage
    ...
    Private Sub MainMenuUserEditedToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles MainMenuUserEditedToolStripMenuItem.Click
        ...
        UserSettings.OCRbackgroundView = BackgroundMode.UserEditedImageArea
    ...
    Private Sub MainMenuUserOrigialToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles MainMenuUserOrigialToolStripMenuItem.Click
        ...
        UserSettings.OCRbackgroundView = BackgroundMode.UserOriginalImageArea
...
#Region "Others"
    ...
    Private Sub ToolStripButton2_Click(sender As Object, e As EventArgs) Handles btnCropBoundaryXX.Click


        If True Then
            Dim img = EditorPicBox.Image.Clone
            img = ImageUtils.ApplyLocalThresholding(img)
            Dim edgptse As New Ag.Filters.SobelEdgeDetector
            edgptse.ApplyInPlace(img)
            Dim edgpts As New Ag.Filters.ConnectedComponentsLabeling

            img = edgpts.Apply(img)

            edgpts.BlobCounter.ObjectsOrder = AForge.Imaging.ObjectsOrder.Size

            Dim ptsRect = edgpts.BlobCounter.GetObjectsRectangles.Last

            img = ImageUtils.CloneImageForGraphics(img)

            Dim img2 = EditorPicBox.Image.Clone
            Using gr = Graphics.FromImage(img2)
                Dim Pen As New Pen(Color.Red, 2)

                gr.DrawRectangle(Pen, ptsRect)

            End Using

            EditorPicBox._image = img2
        ...
        ElseIf False Then
            ...
            ' EditorPicBox.Image = PreProcessor.CropBoundary(EditorPicBox.Image)
