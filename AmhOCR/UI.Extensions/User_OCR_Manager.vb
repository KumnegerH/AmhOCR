...
Partial Class MainWindow
    ...
    'Update progess to the user during OCR process
    Private BatchOCRprog As BatchProgressControl

    ''' <summary>
    ''' Handles single page OCR order
    ''' </summary>
    ...
    Private Async Sub RecognizeFile(ByVal FileName As String)
        ...
        If isRecognizOpen = False Then
            ...
            ' UserSettings.PageSegMode = PageSegMode.fullauto
            ...
            NewPages.PageOCRsettings.Language = UserSettings.Language
        ...
        If NewPages IsNot Nothing Then
            ...
            txtBoxResult.Font = UserSettings.ocrFont.Clone
    ...
    ''' <summary>
    ''' Split unrecognized image into multiple batch and order batch OCRing 
    ''' </summary>
    Private Sub RecognizeBatch()
        ...
        If files.Count > 0 Then
            ...
            If BatchOCRprog.TotalTasks > UserSettings.MaxBatch Then
                BatchOCRprog.NumberOfProcess = UserSettings.MaxBatch
            End If
    ...
    '''  Do batch OCRing 
    ''' </summary>
    ''' <param name="FileNames"List of image file name in a single batch></param>
    ...
    ''' <summary>
    ''' Set batch numbers and number of file in a bacth 
    ''' </summary>
    ...
    Private Function GetProgresGroup(ByVal files As List(Of String)) As List(Of List(Of String))
        ...
        For Each file In files
            ...
            If HocrPages.Where(Function(X) X.ImageName = file).Count = 0 Then
                ...
                NewPages.imgCopyName = Path.Combine(UserSettings.ProjectTempFolder, Path.GetFileName(file))
        ...
        If files.Count > UserSettings.MaxBatch Then

            Dim cnt As Integer = Math.Floor(files.Count / UserSettings.MaxBatch)


            Dim ttc As Integer = 0

            For grp As Integer = 1 To UserSettings.MaxBatch
                ...
                If grp = UserSettings.MaxBatch Then
    ...
    ''' <summary>
    ''' Re-Recognize a bounding box area which was previousely recognized
    ''' </summary>
    ...
    Private Async Sub ReRecognizeBox(ByVal sender As Object, ByVal e As RecognizeAreaArg)
        ...
        UserSettings.PageSegMode = e.pageMode

        Dim NewPages As HocrPage

        NewPages = New HocrPage
        NewPages.ImageName = e.filename
        NewPages.imgCopyName = Path.Combine(UserSettings.ProjectTempFolder, Path.GetFileName(e.filename))
    ...
    ''' <summary>
    ''' Do OCR for an image area selcted by user
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    Private Async Sub RecognizeNewBox(ByVal sender As Object, ByVal e As RecognizeAreaArg)
        ...
        oldPages.PageOCRsettings.Language = UserSettings.Language
    ...
    ''' <summary>
    ''' Remove OCR object from a HOCR data tree
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ...
    ''' <summary>
    ''' Update xml file and data stracture of HOCR
    ''' </summary>
    ''' <param name="filename">Image's file name</param>
    ''' <param name="EdMode">The current edit mode of HOCR</param>
    ...
    ''' <summary>
    ''' Reset image's OCR data and set it to unrecognized state
    ''' </summary>
    ''' <param name="imgMame"></param>
    Private Sub HocrReset(ByVal imgMame As String)
        ...
        If imgIndex >= 0 Then
            If HocrPages.Any(Function(X) X.ImageName = imgMame) Then
                ...
                thisHocrPage.imgCopyName = Path.Combine(UserSettings.ProjectTempFolder, Path.GetFileName(imgMame))
